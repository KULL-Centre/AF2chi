...
for dir in subfolders:
    ...
    md_ensemble_allchis = load_json(f'allproteins_ATLAS_allchis/{pdb}.json')
    ...
    for idx,res in enumerate(reweighted_distributions.keys()):
        if res in md_ensemble_angles_distr.keys():
            js_re.append(jensenshannon(reweighted_distributions[res],md_ensemble_angles_distr[res]))
            js_prior.append(jensenshannon(prior_distributions[res],md_ensemble_angles_distr[res]))
            js_top8000.append(jensenshannon(top8000_prior[f'{res[:3]}_{chi_type}'],md_ensemble_angles_distr[res]))
            js_af2.append(jensenshannon(af2_distributions[res],md_ensemble_angles_distr[res]))

    is_fixed={}
    
    for idx,res in enumerate(reweighted_distributions.keys()):
        if res in md_ensemble_angles_distr.keys():
            red_pop=rot_pop_rebinned_to_three(md_ensemble_angles_distr[res])
    ...
    for idx,res in enumerate(reweighted_distributions.keys()):
        if res in md_ensemble_angles_distr.keys():
            if is_fixed[res]:
                js_re_fixed.append(jensenshannon(reweighted_distributions[res],md_ensemble_angles_distr[res]))
                js_prior_fixed.append(jensenshannon(prior_distributions[res],md_ensemble_angles_distr[res]))
                js_top8000_fixed.append(jensenshannon(top8000_prior[f'{res[:3]}_{chi_type}'],md_ensemble_angles_distr[res]))
                js_af2_fixed.append(jensenshannon(af2_distributions[res],md_ensemble_angles_distr[res]))
    
            else:
                js_re_multi.append(jensenshannon(reweighted_distributions[res],md_ensemble_angles_distr[res]))
                js_prior_multi.append(jensenshannon(prior_distributions[res],md_ensemble_angles_distr[res]))
                js_top8000_multi.append(jensenshannon(top8000_prior[f'{res[:3]}_{chi_type}'],md_ensemble_angles_distr[res]))
                js_af2_multi.append(jensenshannon(af2_distributions[res],md_ensemble_angles_distr[res]))
...
save_json(f'output_js_results/ATLAS_proteins_js_results_{chi_type}.json', js_results_dict)
...
save_json(f'output_js_results/ATLAS_proteins_js_fixedresis_results_{chi_type}.json', js_fixed_results_dict)
...
save_json(f'output_js_results/ATLAS_proteins_js_multiresis_results_{chi_type}.json', js_multi_results_dict)
